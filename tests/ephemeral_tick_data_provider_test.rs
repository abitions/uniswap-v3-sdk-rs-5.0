...
async fn test_retrieve_and_display_all_ticks() -> Result<(), Error> {
    ...
    // USDC/WETH 0.3% 池子地址
    let pool_address = address!("0x6513faae3c52e258e2ca3128010d6e8a79b3c650");
    ...
    // 创建区块ID（可选）
    let block_id = Some(alloy::eips::BlockId::from(41653273));
    ...
    println!(
        "Tick范围: {} 到 {}",
        provider.tick_lower, provider.tick_upper
    );
    println!("Tick间距: {}", provider.tick_spacing);
    println!("获取到的Tick数量: {}", provider.ticks.len());

    // 将ticks数据转换为JSON格式
    let ticks_json: Vec<TickData> = provider
        .ticks
        .iter()
        .map(|tick| TickData {
            tick: tick.index,
    ...
    {
        let json_data =
            serde_json::to_string_pretty(&ticks_json).expect("Failed to serialize JSON");
